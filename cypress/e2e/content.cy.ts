...
import { NextToGo } from '../../src/components/NextToGo';
...
describe('Page Content', () => {
  ...
  //get the category id of the check box 
  //any id attribute (=race.categoryid) in next to go list shouldn't contain this category id
  it('Toggle checkboxes and it should show only selected categories', () => {
      cy.get('[data-testid=category-filters]').then(() => {
        RACING_CATEGORIES.forEach((category) => {
          cy.get(`[data-testid=category-filter-${category.categoryId}]`).then(() => {
              cy.get('[data-testid=category-filter-checkbox]').uncheck();
              console.log(cy.get('[data-testid="Next2go"]').invoke('attr', 'id'));
              cy.get('[data-testid="Next2go"]').invoke('attr', 'id').should('not.contain', category.categoryId)           
          });
        })
      });
